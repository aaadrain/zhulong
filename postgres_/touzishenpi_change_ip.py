...
def f1(driver,num):
    ...
    while int(cnum) != int(num):
        ...
        if " 办理结果公示查询" not in driver.page_source  and flag< 5:
            driver.quit()
            try:
                new_ip = get_ip()
                driver = get_driver(new_ip)
                driver.get(url)
                f4(driver)
                cnum = driver.find_element_by_xpath('//input[@id="pageNum"]').get_attribute('value')
            except:
...
if __name__ == "__main__":
    ...
    work(conp=["postgres", "since2015", "192.168.3.171", "anbang", "touzishenpi"],num=2,headless=False)
